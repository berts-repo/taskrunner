...
/// Serialized in camelCase, the shape of the fixture `expected.json` files the
...
pub fn find_jsonl_files(dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    // Whole-path string order, compared as UTF-16 like the sweeper that wrote
    // the existing archive: it decides the order events land in the log.
