...
- Retired unbuilt with the 2026-07-16 scope cut (knowledge layer,
  instruction packages, project-local state):
  - Project-local directory `.taskrunner/` and its layout
  - Instruction terminology: instruction, instruction package,
    `instruction.toml`, `body.md`, instruction loader, instruction
    snapshot, instruction registry
...
- `session` is the durable record of an MCP client connection.
- `job` remains available for internal queue/execution semantics if needed.
- `integration` describes optional setup for clients or workers such as
  Codex and Claude. `configured capability` describes the same idea from a
