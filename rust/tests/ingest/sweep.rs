...
fn sweeps_a_large_transcript_completely() {
    // The TypeScript test bounds the longest event-loop stall here; the Rust
    // sweep runs on a blocking thread and the socket is served on others, so
    // what remains to check is that a big single file is swept whole.
    let h = harness();
