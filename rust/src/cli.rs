//! The `taskrunner` command line. Commands and their output are frozen from
//! the TypeScript CLI, usage text included, so arguments are parsed by hand
//! the same way: a command, positionals, and `--key value` flags anywhere.
