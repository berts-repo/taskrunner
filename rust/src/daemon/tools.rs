...
//! frozen contract with Claude Code, so they are data — `tools.json`, the
//! TypeScript server's own `tools/list` output — served verbatim and used to
...
impl ToolTable {
    ...
    /// in the schema's property order, the shape the TypeScript server
    /// audited (zod emits parsed objects in schema order).
