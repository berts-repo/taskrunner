// One two-turn task, then lookup assertions against it. The events are the
// ones the scheduler wrote for that flow, applied straight to the index; the
// whole stack is driven in tests/scheduler/integration.rs.
